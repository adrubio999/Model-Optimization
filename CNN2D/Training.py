...
for n_channels in n_channels_arr:
    ...
    for window_size in window_size_arr:
        ...
        for timesteps in window_seconds:
            ...
            for n_config,conf in enumerate(conf_arr):
                print('\n',conf)
                # Nº de épocas
                for n_epochs in n_epochs_arr:
                    ...
                    for n_train_batch in n_train_batch_arr:
                        print('Nº de canales: %d, Duración de la ventana: %d, Time steps:%d, Nº de configuración: %d,\n Nº de épocas: %d, Train Batch Size: %d' % (n_channels,window_size,timesteps,n_config,n_epochs,n_train_batch))
                        
                        model = mb.model_builder_prob(conf, input_shape = input_shape, 
                                        learning_rate  = 1e-5)
